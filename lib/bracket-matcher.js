...
class BracketMatcher {
  constructor (editor, editorElement, matchManager) {
    ...
    this.subscriptions.add(
      atom.commands.add(editorElement, 'bracket-matcher-custom:remove-brackets-from-selection', event => {
  ...
  insertText (text, options) {
    ...
    if (text === '#' && this.isCursorOnInterpolatedString()) {
      autoCompleteOpeningBracket = this.getScopedSetting('bracket-matcher-custom.autocompleteBrackets') && !hasEscapeCharacterBeforeCursor
      text += '{'
      pair = '}'
    } else {
      autoCompleteOpeningBracket = (
        ...
        !hasWordAfterCursor &&
        this.getScopedSetting('bracket-matcher-custom.autocompleteBrackets') &&
    ...
    if (this.isClosingBracket(text) && (nextCharacter === text) && !hasEscapeCharacterBeforeCursor) {
      ...
      if (bracketMarker || this.getScopedSetting('bracket-matcher-custom.alwaysSkipClosingPairs')) {
  ...
  wrapSelectionInBrackets (bracket) {
    ...
    if (!this.getScopedSetting('bracket-matcher-custom.wrapSelectionsInBrackets')) return false
